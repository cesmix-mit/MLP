...
function empiricalperatomdescriptors(x, q, t, a, b, c, pbc, rcutmax, eta, kappa, emdescriptors)
    ...
    for i = 1:length(emdescriptors)        
        ...
        if (style == "single")
            ...
            fa = zeros(dim,N,N,M)
            if length(dd)==0
                d = 1.0*reshape(ea, (N, M)); 
                dd = 1.0*fa;
            else
                d = cat(d, ea, dims=2)   # num_atoms by num_descriptors
                dd = cat(dd, fa, dims=4)
            end    
        end
        ...
        if (style == "pair")
            ...
            eij, fij = potentialfunc(xij, qi, qj, ti, tj, mu, eta, kappa);     
            for m = 1:size(eij,2)
                ea, fa = tallypair(eij[:,m], fij[:,:,m], ai, aj, pairnum);      
                if length(dd)==0
                    d = 1.0*ea; 
                    dd = 1.0*fa;
                else
                    d = cat(d, ea, dims=2)   # num_atoms by num_descriptors
                    dd = cat(dd, fa, dims=4) # dim by num_atoms by num_atoms by num_descriptors
                end                    
            end
        end
        ...
        if (style == "triplet")
            ...
            eij, fij, fik = potentialfunc(xij, xik, qi, qj, qk, ti, tj, tk, mu, eta, kappa);                                
            for m = 1:size(eij,2)
                ea, fa = tallytriplet(eij[:,m], fij[:,:,m], fik[:,:,m], ai, aj, ak, tripletnum);                                             
                if length(dd)==0
                    d = 1.0*ea; 
                    dd = 1.0*fa;
                else
        ...
        if (style == "quadlet")
            ...
            eij, fij, fik, fil = potentialfunc(xij, xik, xil, qi, qj, qk, ql, ti, tj, tk, tl, mu, eta, kappa);            
            for m = 1:size(eij,2)                
                ea, fa = tallyquadlet(eij[:,m], fij[:,:,m], fik[:,:,m], fil[:,:,m], ai, aj, ak, al, quadletnum);                            
                if length(dd)==0
                    d = 1.0*ea; 
                    dd = 1.0*fa;
                else
