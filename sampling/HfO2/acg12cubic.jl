...
aranges = [80 100; 80 100; 80 100];        
adims = [23, 23, 23];        
alattices, aatombases = ACG.angleconfigurations(lat, aranges, adims, descriptors, 1500);

lranges = [a-da a+da; b-db b+db; c-dc c+dc; 80 100; 80 100; 80 100];        
ldims = [6, 6, 6, 4, 4, 4];        
...
dlattices, datombases = ACG.selectconfigurations(la[1:3,:], la[4:6,:], la[7:9,:], ba[:,:,:,1], atomtype, descriptors, 1e-4);
...
ACG.writeEXTXYZ2("HfO2sg225n12.xyz", species, natoms, lattices, atomtypes, atombases);
