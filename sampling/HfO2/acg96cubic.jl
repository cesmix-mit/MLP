...
fraccoords = [0.000000 0.000000 0.000000 
...
atomtypes = reshape(atomtypes, (1, 12*m*n*p));
atomtype = atomtypes[:];
...
pranges = [a-da a+da; b-db b+db; c-dc c+dc];       
pdims = [23, 23, 23];        
...
lranges = [a-da a+da; b-db b+db; c-dc c+dc; 75 105; 75 105; 75 105];        
ldims = [6, 6, 6, 4, 4, 4];        
...
dlattices, datombases = ACG.selectconfigurations(la[1:3,:], la[4:6,:], la[7:9,:], ba[:,:,:,1], atomtype, descriptors, 420);

natom = length(atomtype);
ds = 0.0125*ones(natom);        
N = 12000;    
blattices, batombases = ACG.atomdisplacementconfigurations(lat, ds, N, descriptors, 580);
n = size(batombases,3)
...
ACG.writeEXTXYZ2("HfO2sg225n96.xyz", species, natoms, lattices, atomtypes, atombases);
