...
for j = 0:5    
    ...
    acedescriptors[1] = ACEpot.ACEparams(species = [:Li], nbody=1, pdegree=1, r0=ACEpot.rnn(:Li), rcut=5.0, rin=1.0, wL=2.0, csp=1.75)
    if j == 0
        acedescriptors[2] = ACEpot.ACEparams(species = [:Li], nbody=3, pdegree=6, r0=ACEpot.rnn(:Li), rcut=rcut, rin=rin, wL=2.0, csp=1.75)
    elseif j == 1
