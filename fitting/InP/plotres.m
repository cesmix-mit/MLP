...
eerr = 1e3*[ace(:,3) snap(1:5,2) snap(6:10,2) pod(1:5,3) pod(6:10,3)];
ferr = 1e3*[ace(:,4) snap(1:5,3) snap(6:10,3) pod(1:5,4) pod(6:10,4)];
...
axis([5 95 0.05 70]);
set(gca, 'YTick', [0.05 0.1 0.2 0.4 0.8 1.6 3.2 6.4 12.8 25.6 51.2])
print -dpng InP_train_energy_error.png
...
semilogy(num(:,1), eerr(:,1),'-o', 'LineWidth', 2, 'MarkerSize', 10); 
hold on
semilogy(num(:,2), eerr(:,3), '--s', 'Color', [0.6350 0.0780 0.1840], 'LineWidth', 2, 'MarkerSize', 10); 
semilogy(num(:,3), eerr(:,4), '-.d', 'Color', [0.4660 0.6740 0.2880], 'LineWidth', 2, 'MarkerSize', 10); 
semilogy(num(:,3), eerr(:,5), '-.h', 'Color', [0.9290 0.5940 0.0250], 'LineWidth', 2, 'MarkerSize', 10); 
set(gca,'FontSize',16); 
set(gca,'LooseInset',get(gca,'TightInset'))
xlabel("Number of basis functions", 'FontSize', 18);
ylabel("Training error in energy (meV/atom)", 'FontSize', 18);
leg = legend({'ACE','SNAP','POD-I','POD-II'}, 'FontSize', 16, 'Location', 'NE');
leg.ItemTokenSize = [50,10];
axis([5 94 0.05 32]);
set(gca, 'YTick', [0.05 0.1 0.2 0.4 0.8 1.6 3.2 6.4 12.8 25.6])
print -dpng InP_train_energy_error.png

figure(2); clf; 
semilogy(num(:,1), ferr(:,1),'-o', 'LineWidth', 2, 'MarkerSize', 10); 
hold on
semilogy(num(:,2), ferr(:,3), '--s', 'Color', [0.6350 0.0780 0.1840], 'LineWidth', 2, 'MarkerSize', 10); 
semilogy(num(:,3), ferr(:,4), '-.d', 'Color', [0.4660 0.6740 0.2880], 'LineWidth', 2, 'MarkerSize', 10); 
semilogy(num(:,3), ferr(:,5), '-.h', 'Color', [0.9290 0.5940 0.0250], 'LineWidth', 2, 'MarkerSize', 10); 
set(gca,'FontSize',16); 
set(gca,'LooseInset',get(gca,'TightInset'))
xlabel("Number of basis functions", 'FontSize', 18);
...
leg = legend({'ACE','SNAP','POD-I','POD-II'}, 'FontSize', 16, 'Location', 'NE');
leg.ItemTokenSize = [50,10];
axis([5 94 0.04*1e2 1.2e2]);
set(gca, 'YTick', [5 10 20 40 80])
print -dpng InP_train_force_error.png
...
% filename = ['results/optpodlossfunction' num2str(10) '.txt'];
% fileID = fopen(filename,'r');
...
xlabel('Inner cut-off distance', 'FontSize',16);
ylabel('Outer cut-off distance', 'FontSize',16);
hold on; plot3(u(1), u(2), u(3), 'or', 'MarkerSize', 8, 'LineWidth', 2, 'MarkerFace', 'r');
colorbar("FontSize",14);
shading interp 
print('-dpng', ['podlossfunction' num2str(3) '.png'])
