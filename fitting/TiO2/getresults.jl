...
podnum = zeros(5)
for j = 1:5
    podcoeff = readdlm("results/optpodcoeff" * string(j-1) *  ".txt")    
    podnum[j] = length(podcoeff)

    tm = readdlm("results/optpodtrainerror" * string(j-1) *  ".txt")    
    podtrain[j,:] = [tm[1,1], tm[1,3]]

    tm = readdlm("results/optpodtesterror" * string(j-1) *  ".txt")    
...
acenum = zeros(5)
for j = 1:5
    acecoeff = readdlm("results/optacecoeff" * string(j-1) *  ".txt")    
    acenum[j] = length(acecoeff)

    tm = readdlm("results/optacetrainerror" * string(j-1) *  ".txt")    
    acetrain[j,:] = [tm[1,1], tm[1,3]]

    tm = readdlm("results/optacetesterror" * string(j-1) *  ".txt")    
...
pod23num = zeros(5)
for j = 1:5
    pod23coeff = readdlm("results/optpod23coeff" * string(j-1) *  ".txt")    
    pod23num[j] = length(pod23coeff)

    tm = readdlm("results/optpod23trainerror" * string(j-1) *  ".txt")    
    pod23train[j,:] = [tm[1,1], tm[1,3]]

    tm = readdlm("results/optpod23testerror" * string(j-1) *  ".txt")    
...
snapnum = zeros(5)
for j = 1:5
    snapcoeff = readdlm("results/optsnapcoeff" * string(j-1) *  ".txt")    
    snapnum[j] = length(snapcoeff)

    tm = readdlm("results/optsnaptrainerror" * string(j-1) *  ".txt")    
    snaptrain[j,:] = [tm[1,1], tm[1,3]]

    tm = readdlm("results/optsnaptesterror" * string(j-1) *  ".txt")    
