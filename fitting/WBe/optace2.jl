...
for j = 3:3
    ...
    elseif j==2
        ...
        descriptors[1] = ACEpot.ACEparams(species = [:W,:Be], nbody=3, pdegree=4, r0=ACEpot.rnn(:Be), rcut=rcut, rin=rin, wL=2.0, csp=1.75)
    ...
    writedlm("results/optace2coeff" * string(j) *  ".txt", coeff)
    writedlm("results/optace2polycoeff" * string(j) *  ".txt", polycoeff)
    writedlm("results/optace2trainerror" * string(j) *  ".txt", [energyerrors forceerrors])    
    #writedlm("results/optpod23testerror" * string(j) *  ".txt", [energytesterrors forcetesterrors])    
    writedlm("results/optace2eta" * string(j) *  ".txt", opteta)
end
