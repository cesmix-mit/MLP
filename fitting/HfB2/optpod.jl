...
for j = 0:0
    ...
    writedlm("results/optpodBcoeff" * string(j) *  ".txt", coeff)
    writedlm("results/optpodBpolycoeff" * string(j) *  ".txt", polycoeff)
    writedlm("results/optpodBtrainerror" * string(j) *  ".txt", [energyerrors forceerrors])    
    writedlm("results/optpodBtesterror" * string(j) *  ".txt", [energytesterrors forcetesterrors])    
    writedlm("results/optpodBeta" * string(j) *  ".txt", opteta)
end
