...
for j = 0:5    
    ...
    acedescriptors[1] = ACEpot.ACEparams(species = [:Cu], nbody=1, pdegree=1, r0=ACEpot.rnn(:Cu), rcut=5.0, rin=1.0, wL=2.0, csp=1.75)
    if j == 0
        acedescriptors[2] = ACEpot.ACEparams(species = [:Cu], nbody=3, pdegree=6, r0=ACEpot.rnn(:Cu), rcut=rcut, rin=rin, wL=2.0, csp=1.75)
    elseif j == 1
        acedescriptors[2] = ACEpot.ACEparams(species = [:Cu], nbody=4, pdegree=8, r0=ACEpot.rnn(:Cu), rcut=rcut, rin=rin, wL=1.75, csp=1.5)
    elseif j==2
        acedescriptors[2] = ACEpot.ACEparams(species = [:Cu], nbody=2, pdegree=3, r0=ACEpot.rnn(:Cu), rcut=rcut, rin=rin, wL=1.5, csp=1.5)
        acedescriptors[3] = ACEpot.ACEparams(species = [:Cu], nbody=4, pdegree=10, r0=ACEpot.rnn(:Cu), rcut=rcut, rin=rin, wL=1.25, csp=1.5)
    elseif j==3 
        acedescriptors[2] = ACEpot.ACEparams(species = [:Cu], nbody=2, pdegree=3, r0=ACEpot.rnn(:Cu), rcut=rcut, rin=rin, wL=1.5, csp=1.5)
        acedescriptors[3] = ACEpot.ACEparams(species = [:Cu], nbody=4, pdegree=12, r0=ACEpot.rnn(:Cu), rcut=rcut, rin=rin, wL=1.5, csp=1.5)
    elseif j==4
        acedescriptors[2] = ACEpot.ACEparams(species = [:Cu], nbody=2, pdegree=12, r0=ACEpot.rnn(:Cu), rcut=rcut, rin=rin, wL=1.5, csp=1.5)
        acedescriptors[3] = ACEpot.ACEparams(species = [:Cu], nbody=4, pdegree=12, r0=ACEpot.rnn(:Cu), rcut=rcut, rin=rin, wL=1.35, csp=1.25)
    elseif j==5           
        acedescriptors[2] = ACEpot.ACEparams(species = [:Cu], nbody=4, pdegree=13, r0=ACEpot.rnn(:Cu), rcut=rcut, rin=rin, wL=1.15, csp=1.25)
    end
